import { BlogPost } from "@/types";

export const allPosts: BlogPost[] = [
  {
    slug: "maximizing-scrap-value",
    title: "5 Tips for Maximizing Your Scrap Metal Value",
    date: "September 28, 2025",
    author: "K&L Team",
    category: "Tips & Guides",
    excerpt:
      "Learn how proper sorting and preparation can significantly increase the revenue you get from your scrap metal.",
    image: "/images/project-planning-scrap.jpg",
    content: "Full blog post content goes here...",
  },
  {
    slug: "demolition-safety-protocols",
    title: "Safety First: Our Process for On-Site Demolition Recycling",
    date: "September 15, 2025",
    author: "K&L Team",
    category: "Safety",
    excerpt:
      "Discover the meticulous planning and safety protocols that go into every K&L demolition project to ensure a safe and efficient process.",
    image: "/images/demolition-safety.png",
    content: "Full blog post content goes here...",
  },
  {
    slug: "sustainability-impact",
    title: "The Environmental Impact of Metal Recycling",
    date: "August 30, 2025",
    author: "K&L Team",
    category: "Sustainability",
    excerpt:
      "Understanding how recycling scrap metal contributes to environmental conservation and reduces carbon emissions.",
    image: "/images/Construction.jpg",
    content: "Full blog post content goes here...",
  },
];
